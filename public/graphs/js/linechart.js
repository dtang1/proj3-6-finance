...
Raphael.fn.lineChart = function(opts, dat) {
	var svg = r = this;
	...
	var width = svg.width,
		height = svg.height,
		leftgutter = width*0.01,
		bottomgutter = height*0.09,
		topgutter = height*0.03,
		colorhue = .6 || Math.random(),
		color = "hsl(" + [colorhue, .5, .5] + ")",
		txt = opts.labelText,
		txt1 = {font: '10px Helvetica, Arial', fill: "#00f"},
		txt2 = {font: '12px Helvetica, Arial', fill: "#000"},
		X = (width - leftgutter) / dat.labels.length,
		max = 0;
	for(i in dat.lines) for(j in dat.lines[i].values) max = Math.max(max, dat.lines[i].values[j]);
	max *= 1.05;
	...
	function drawLine(_j) {
		...
		for (var i = 0, ii = labels.length; i < ii; i++) {
			var y = Math.round(height - bottomgutter - Y * data[i]),
				...
				t = r.text(x, height*0.94, labels[i]).attr(txt).toBack();
			if (!i) {
				p = ["M", x, y, "C", x, y];
